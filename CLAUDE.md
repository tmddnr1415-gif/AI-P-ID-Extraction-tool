...
| **Description 생성** | 진행 중 — 발주처 표기 규칙 정리 후 **F1 71.8 · 완전일치 32행** |
...
| 조각 | 값 | 비고 |
| --- | --- | --- |
| 발주처 557행 대조 | 완전일치 **32** · 부분 486 · 미달 0 · 대응행 없음 39 | 직전 25 / 493 / 0 |
| 토큰 정밀도 / 재현율 / F1 | **73.4% / 70.3% / 71.8** | 직전 60.1 / 67.5 / 63.6 |
| **주어 귀속** | 완전 **14.5%** (75/518) · 대부분 이상 **62.4%** | 직전 7.9% / 60.0% |
| 변수어 | **99.0%** (501/506) | |
| 기기명 | **78.4%** (287/366) | 직전 78.1% |
| 순번 | 35.7% (96/269) | 직전 48.0% — 라인 문구를 뺀 162행이 주어와 함께 순번도 잃음 |
| 위치어 | **25.9%** (41/158) — 오답 36 · 미부착 81 | 직전 24.7% |
| 중간 심볼 | **0%** (0/13) | 회수 불가 확정 — §3 참조 |
| 등급 | CONFIRMED 616 · **PARTIAL 112** · LOW 18 · NONE 144 · SKIP 2 | PARTIAL 은 라인 문구를 뺀 행 |
| 서로 다른 문장 | 490 | 발주처도 557행 중 110행(19.7%)이 같은 문장을 반복 |
| 목표 도면 p33 23행 | 재현 **12** · 부분 11 · 실패 0 | |
| 표본 3계통 15행 | 재현 **5** · 부분 10 | 직전 재현 2 |
...
1. **기기 라벨을 못 찾는 도면** — 주어 완전 오답 50행의 원인은 ① 그 도면에 기기
   후보가 **하나도** 없음 19행 (예: p20 `11LAB00`, 발주처는 `BOILER FEED WATER
   PUMP` 라고 씀) · ② 정답 기기가 후보에 있는데 순위가 밀림 19행 · ④ 정답 기기가
   후보에 아예 없음(라벨 미검출) 12행 · ③ 라벨 표기가 발주처와 다름 0행.
   **①과 ④(31행)는 라벨 검출 문제**이고, ②(19행)만 순위 문제입니다.
2. **위치어 158행 중 117행이 아직 비었거나 틀립니다.** 과반이 나오는 칸은 5개뿐
...
3. **순번이 35.7% 로 내려갔습니다** — 라인 문구를 뺀 162행이 주어를 잃으면서 순번도
   함께 잃었습니다. 이 행들에 기기 라벨을 찾아 주면 순번도 같이 돌아옵니다(과제 1과 동일).
4. **LS 문구 안의 유닛 번호** — 발주처는 `TO HRSG BD TANK` 라고 쓰는데 우리는
   `TO HRSG#11 BD TANK` 라고 씁니다. 문장 머리에 이미 `UNIT #11` 이 있으므로
   중복입니다. 18행. 557행에서 확인 가능.
5. **중간 심볼(SUCTION STRAINER)** — 발주처 13행, 전부 PDIT, 정확도 **0/13**.
...
6. **밸브 Description 144행** — 발주처 마스터 밸브 리스트가 없어 문형 기준이 없음.
...
7. **다중 신호 버블 102행** — 합산 여부가 발주처 확인 대기
   (`out/ls_bundle_question.md`, `config multi_signal_bundle.merge_quantity`).
8. **UNKNOWN 6종** (`out/valve_project_deps.md`) — Phase 3.
9. **Tag No.** — 도면이 `.....` 로 미부여. 발주처 규칙 필요.
