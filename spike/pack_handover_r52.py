"""회사 PC 갈음 꾸러미 r52 를 만든다 (10~52회차 누적).

    python3 spike/pack_handover_r52.py
...
바깥 zip (`out/PID_update_<날짜>_r52.zip`) 에는 넷이 들어간다
    0_report.md             <- 반영 사항 + 다음 개발 사항 (저장소 뿌리의 report.md)
    4_result.md             <- 이번 회차 개선 결과 (out/round52/result.md)
    1_먼저읽기_README.txt   <- out/round52/1_먼저읽기_README.txt (사람이 쓴 안내)
    2_파일목록.txt          <- [A]/[M] 목록
...
def main() -> int:
    ...
    for must in (readme, report, result):
        if not must.exists():
