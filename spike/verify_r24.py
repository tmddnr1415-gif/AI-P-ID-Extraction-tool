"""갈음 r24 — 회사 PC 조건 재현 적용 검증 (13단계).

    python3 spike/verify_r24.py <단계>
...
WORK = Path("/tmp/v24")
DATA = Path("/tmp/v24_data")
OUT = ROOT / "out" / "round24" / "verify"
BASE = "aac64ba"
...
def stage5():
    ...
    import zipfile
    pkg = ROOT / "out" / "round24" / "3_FILES_TO_OVERWRITE.zip"
...
def stage9():
    ...
    srv = Server(free_port())
    try:
        snap = srv.send("/jobs/%s/snapshot" % prev["job2"], {"label": "검증 r24"})
