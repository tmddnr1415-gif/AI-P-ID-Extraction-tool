"""꾸러미 r26 — 회사 PC 조건 재현 적용 검증 (14단계).

    python3 spike/verify_r26.py <단계>
...
def stage5():
    ...
    import zipfile
    pkg = ROOT / "out" / "round26" / "3_FILES_TO_OVERWRITE.zip"
...
def stage9():
    ...
    srv = Server(free_port())
    try:
        snap = srv.send("/jobs/%s/snapshot" % prev["job2"], {"label": "검증 r26"})
