"""꾸러미 r27 — 회사 PC 조건 재현 적용 검증 (14단계).

    python3 spike/verify_r27.py <단계>
...
WORK = Path("/tmp/v26")
DATA = Path("/tmp/v26_data")
OUT = ROOT / "out" / "round27" / "verify"
BASE = "aac64ba"
...
def stage5():
    ...
    import zipfile
    pkg = ROOT / "out" / "round27" / "3_FILES_TO_OVERWRITE.zip"
...
def stage9():
    ...
    srv = Server(free_port())
    try:
        snap = srv.send("/jobs/%s/snapshot" % prev["job2"], {"label": "검증 r27"})
