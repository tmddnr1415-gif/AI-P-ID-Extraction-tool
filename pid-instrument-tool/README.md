...
**https://tmddnr1415-gif.github.io/AI-P-ID-Extraction-tool/**
...
|---|---|---|
| 주소 | 공개 (누구나) | 공개 + **비밀번호** |
| API 키 | 쓰는 사람이 각자 입력 | **서버에 보관**, 안 내려감 |
| Excel 템플릿 | 쓰는 사람이 업로드 | 서버가 내려줌 |
| 사용량 한도 | 각자 자기 계정 | 하루 N장 (KV) |
| 배포 | 이미 됨 | `cd server && ./deploy.sh` |

**팀에 나눠줄 때는 공유 링크 서버**가 맞습니다. 받는 사람이 API 키도 템플릿도 준비할
필요가 없습니다. GitHub Pages는 **혼자 쓰거나 남에게 보여줄 때** 편합니다.

올라가는 것은 브라우저에서 도는 앱뿐입니다. 도면 PDF와 Excel 템플릿, API 키는
쓰는 사람 브라우저 밖으로 나가지 않습니다(판독 요청은 그 브라우저에서 Anthropic으로
직접 갑니다). 다만 **이 저장소는 공개**라 `rules/` 의 판독 규칙 — 도면번호와 설비
명명 규칙, 벤더 공급 범위 같은 프로젝트 정보 — 도 함께 공개됩니다.
...
### DESCRIPTION 자동 생성을 시도했다가 접은 이유

재료는 텍스트 레이어에 다 있습니다. 설비 박스 이름(`#11 BFP A/B COOLERS`), 배관
행선지(`TO HRSG#11 HP ECONOMIZER`), 측정 대상(TYPE에서 나옴), 호기 번호까지.
가장 가까운 라벨을 주워 조립하는 방식을 만들어 정답과 대조했습니다.

| 도면 | 완전일치 |
|---|---|
| p38 CCW (갈래마다 설비 박스가 있는 도면) | 6 / 24 |
| p6 HP Steam | 0 / 12 |
| p20 Feedwater | 0 / 33 |
| **합계** | **6 / 69 (9%)** |

p20 정답은 `UNIT #11 BOILER FEED WATER PUMP A DISCHARGE HP FLOW A` 인데, 도면에는
`BOILER FEED WATER PUMP` 라는 글자가 없습니다(`FEEDWATER PUMP FOR HRSG UNIT #11`).
`DISCHARGE HP` 도 그 배관이 HP 이코노마이저로 간다는 걸 따라가야 나옵니다.
p6 정답의 `HP STEAM` 은 설비명이 아니라 계통 제목이고, `DRAIN` 은 그 DN300 배관이
드레인이라는 걸 알아야 합니다.

**어느 라벨을 고를지와 뭐라고 부를지가 곧 판독입니다.** 좌표만으로는 안 됩니다.
9% 정확도의 문장을 그럴듯하게 채워 넣는 것은 안전 관련 문서에서 위험하다고 판단해
**문장을 지어내지 않습니다.**

대신 각 행의 **근거** 칸에 그 계기 옆에 있던 **설비명과 배관 행선지를 붙여 줍니다.**
도면을 다시 뒤지지 않고 검토 UI에서 바로 쓸 수 있습니다.
