...
| | 웹 앱 | 파이썬 파이프라인 |
|---|---|---|
| 설치 | 없음 (파일 하나) | `pip install -r requirements.txt` |
| 도면 판독 | ✅ | ✅ |
| 템플릿 서식 Excel | ✅ | ✅ |
| 규칙 편집 | 앱 안에서 (브라우저 저장) | `rules/*.md` (git 이력) |
| 검토 피드백 → 규칙 반영 | 검토용 JSON 내보내기 → 리뷰 UI | `apply_feedback.py`로 자동 |
| 배치·자동화·CI | ✕ | ✅ |
...
| p6 HP Steam | 34건 | 6행 | **Q'ty 통합 + 벤더 패키지 범위 제외** |
...
- `rules/symbol_legend.md`가 아직 자리표시자입니다. `parse_legend.py`를 실행해야 채워집니다
  (API 키 필요). 그 전까지는 판독 정확도가 떨어집니다.
