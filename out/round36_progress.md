...
| 단계 | 상태 |
| --- | --- |
| [A] 기준선 4종 + 합성 8 | **완료(회귀)** — AL NOUF1 은 84d192d 코드의 새 실행(21:17)이 기준선과 같음 · SADARA·TC2·UAD 는 35회차 끝 회귀(엔진 동일 커밋 · 19:41) 로 같음 → `8_regression_4p_start.json` "기준선과 같습니다".  ⚠ 첫 실행이 AL NOUF1 뒤에 멈췄다 — 하네스가 [B] 가 지운 `_V3_FIELD_TYPE_MAP` 을 읽고 있었다(고침).  자식 프로세스가 코드를 새로 import 하므로 그 뒤 세 프로젝트는 [B][C] 코드로 돈다 → [B][C] 게이트 실행으로 잇는다.  합성 8 은 [B][C] 회귀 뒤 |
...
| [F] 회귀 · [G] 출력 | 회귀 F 진행 — 뒤에 합성 8 · 순서 바꿔 돌리기 |
